...
from dipper.__main__ import _parse_config, _resolve_groups
...
class TestParseConfig:
    def test_missing_file_returns_empty(self):
        tokens, presets = _parse_config(Path("/nonexistent/path/config"))
        assert tokens == []
        assert presets == {}

    def test_blank_lines_ignored(self):
        path = write_config("\n\n--prompt foo\n\n")
        tokens, presets = _parse_config(path)
...
class TestResolveGroups:
    def _ns(self, groups=None, preset=None):
        import argparse
    ...
    def test_no_groups_no_preset_returns_none(self):
        assert _resolve_groups(self._ns(), {}) is None

    def test_groups_returned_directly(self):
        assert _resolve_groups(self._ns(groups="bug,critical"), {}) == "bug,critical"

    def test_preset_expands_to_csv(self):
        presets = {"testing": "bug,critical,security"}
        result = _resolve_groups(self._ns(preset="testing"), presets)
        assert result == "bug,critical,security"

    def test_groups_wins_over_preset(self):
        presets = {"testing": "bug,critical"}
        result = _resolve_groups(self._ns(groups="custom", preset="testing"), presets)
        assert result == "custom"

    def test_unknown_preset_exits(self):
        with pytest.raises(SystemExit):
            _resolve_groups(self._ns(preset="nonexistent"), {})
